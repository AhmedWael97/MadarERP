/**
 * Customer Categories list — matches reference Blade
 *   `H:/coupons/Madaar ERP/Madaar ERP/resources/views/customers/categories/index.blade.php`.
 *
 * Compact 4-column table backed by the `Madaar Customer Category` doctype.
 */
...
export default function CustomerCategoryListPage() {
  return (
    <RequirePerm doctype="Madaar Customer Category" action="read">
      <PageShell
        title="تصنيفات العملاء"
        subtitle="إدارة تصنيفات وفئات العملاء"
        actions={
          <>
            <Link
              to="/customers"
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-300 text-sm font-semibold rounded-xl transition-all"
            >
              العملاء
            </Link>
            <Link
              to="/customer-categories/create"
...
function CategoryTable() {
  ...
  async function onDelete(name: string) {
    ...
    try {
      await deleteDoc('Madaar Customer Category', name);
            ...
            {(rows ?? []).map((c) => (
              <tr key={c.name} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                ...
                {!hide('discount_percentage') && (
                  <td className="px-5 py-3 text-sm font-mono" dir="ltr">
                    {fmtNum(c.discount_percentage ?? 0)}%
                  </td>
                )}
                {!hide('is_active') && (
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center justify-center gap-1">
                    <button
                      type="button"
                      onClick={() => navigate(`/customer-categories/${encodeURIComponent(c.name)}/edit`)}
                      className="p-1.5 rounded-lg text-slate-400 hover:text-[color:var(--color-brand-600)] hover:bg-[color:var(--color-brand-50,#ecfdf5)] dark:hover:bg-[color:var(--color-brand-500)]/10 transition"
                      aria-label="edit"
                    >
                      <Pencil size={16} />
                    </button>

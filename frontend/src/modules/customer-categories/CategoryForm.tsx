/**
 * Customer Category create / edit form — matches reference Blade
 *   `H:/coupons/Madaar ERP/Madaar ERP/resources/views/customers/categories/form.blade.php`.
 *
 * Single card with 3 fields (name_ar, name_en, discount_percentage) + is_active toggle.
 */
...
export default function CategoryFormPage({ mode }: { mode: 'create' | 'edit' }) {
  ...
  return (
    <RequirePerm doctype="Madaar Customer Category" action="read">
      <PageShell
        title={isEdit ? 'تعديل تصنيف' : 'إضافة تصنيف عملاء'}
      ...
      >
        <CategoryFormBody mode={mode} name={id} onDone={() => navigate('/customer-categories')} />
      </PageShell>
    </RequirePerm>
  );
}
...
function CategoryFormBody({
  ...
  const { data: existing } = useFrappeGetDoc<CategoryDoc>(
    ...
    isEdit && name ? `category:${name}` : null,
  );
  useEffect(() => {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit && name) {
        await updateDoc('Madaar Customer Category', name, cleaned);
        toast.success('تحديث');
      } else {
        await createDoc('Madaar Customer Category', cleaned);
        toast.success('تم الحفظ');
      }
      onDone();
    } catch (e: any) {
      const sm = e?._server_messages ?? e?.message;
      toast.error(typeof sm === 'string' ? sm : 'تعذر الحفظ');
    }
  }
        ...
        <div className="p-6 space-y-4">
          ...
          <label className="inline-flex items-center gap-2 cursor-pointer">
            <input
              ...
              className="w-5 h-5 rounded border-slate-300 text-[color:var(--color-brand-500)] focus:ring-[color:var(--color-brand-500)]"
            />

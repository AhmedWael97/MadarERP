/**
 * SalesDocumentForm — shared form for the 4 sales transaction doctypes:
 *   Sales Invoice / Sales Order / Quotation / Sales Invoice with is_return=1.
 *
 * Mirrors reference Blade `sales/{invoices,orders,quotations,returns}/form.blade.php`
 * which all share the same layout:
 *   1. Header card: customer, project, sales rep, payment method, warehouse,
 *      dates, cost center, notes
 *   2. Items table (delegated to the existing <LineItemsTable> + react-hook-form
 *      child-array machinery — same one FormShell uses, so totals roll up).
 *   3. Totals card (subtotal / discount / tax / grand total — driven from the
 *      doc's own running totals that ERPNext maintains as `total`, `discount_amount`,
 *      `total_taxes_and_charges`, `grand_total`).
 *   4. Footer: cancel + brand gradient submit.
 *
 * Switching doctype just changes the title, the create/update target, and one
 * or two field labels (e.g. "فاتورة" → "أمر بيع" → "عرض سعر").
 */
...
export type SalesVariant = 'invoice' | 'order' | 'quotation' | 'return';
...
interface VariantConfig {
  ...
  subtitle: string;
  listPath: string;
  /** Quotations have `party_name` instead of `customer`. */
  customerField: 'customer' | 'party_name';
  /** Returns set is_return=1 on the underlying doc. */
  isReturn?: boolean;
  /** Some doctypes use `transaction_date` not `posting_date`. */
  dateField: 'posting_date' | 'transaction_date';
}

const VARIANTS: Record<SalesVariant, VariantConfig> = {
  invoice:   { doctype: 'Sales Invoice', titleNew: 'فاتورة مبيعات جديدة', titleEdit: 'تعديل فاتورة',  subtitle: 'فاتورة المبيعات',   listPath: '/sales/invoices',   customerField: 'customer',   dateField: 'posting_date' },
  order:     { doctype: 'Sales Order',   titleNew: 'أمر بيع جديد',        titleEdit: 'تعديل أمر بيع',  subtitle: 'أوامر البيع',        listPath: '/sales/orders',     customerField: 'customer',   dateField: 'transaction_date' },
  quotation: { doctype: 'Quotation',     titleNew: 'عرض سعر جديد',        titleEdit: 'تعديل عرض السعر', subtitle: 'عروض الأسعار',      listPath: '/sales/quotations', customerField: 'party_name', dateField: 'transaction_date' },
  return:    { doctype: 'Sales Invoice', titleNew: 'مرتجع مبيعات جديد',   titleEdit: 'تعديل مرتجع',     subtitle: 'مرتجعات المبيعات',  listPath: '/sales/returns',    customerField: 'customer',   dateField: 'posting_date', isReturn: true },
};

export default function SalesDocumentForm({ variant, mode }: { variant: SalesVariant; mode: 'create' | 'edit' }) {
...
function Body({ cfg, variant, mode, name, onDone }: { cfg: VariantConfig; variant: SalesVariant; mode: 'create' | 'edit'; name?: string; onDone: () => void }) {
  const isEdit = mode === 'edit';
  ...
  // Fetch the parent doctype's meta so LineItemsTable can render the items child table.
  ...
  const { data: existing } = useFrappeGetDoc<Record<string, unknown>>(
    ...
    isEdit && name ? `sd:${cfg.doctype}:${name}` : null,
  );
  useEffect(() => {
    if (existing) form.reset(existing);
  }, [existing]);

  // Reference data for the header pickers.
  ...
  const { data: customers } = useFrappeGetDocList<{ name: string; customer_name?: string; madaar_customer_code?: string }>('Customer', {
    fields: ['name', 'customer_name', 'madaar_customer_code'],
    limit: 200,
  });
  const { data: salesPersons } = useFrappeGetDocList<{ name: string }>('Sales Person', { fields: ['name'], limit: 100 });
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Card title="بيانات الفاتورة">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
          <Field label="اسم العميل" required>
            <select required {...form.register(cfg.customerField, { required: true })} className={INPUT}>
              <option value="">— اختر العميل —</option>
              {(customers ?? []).map((c) => (
                <option key={c.name} value={c.name}>
                  {c.madaar_customer_code ? `${c.madaar_customer_code} — ` : ''}{c.customer_name ?? c.name}
                </option>
              ))}
            </select>
          </Field>
          {cfg.customerField === 'party_name' && (
            <input type="hidden" {...form.register('quotation_to')} value="Customer" />
          )}
          <Field label="المسلسل">
          ...
          <Field label="مندوب مبيعات">
            <select {...form.register('madaar_sales_person')} className={INPUT}>
              <option value="">— بدون —</option>
              {(salesPersons ?? []).map((s) => (<option key={s.name} value={s.name}>{s.name}</option>))}
            </select>
          </Field>
      ...
      {/* Items table — delegate to the existing LineItemsTable */}
      <div className="bg-white dark:bg-slate-800/50 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 overflow-hidden">
        ...
        <div className="p-4">
          {itemsField ? (
            ...
            <p className="text-sm text-slate-500 text-center py-6">جاري تحميل خصائص جدول الأصناف...</p>
          )}
        </div>
      </div>

/**
 * SalesDocumentList — shared list view for the 4 sales transaction lists.
 * Stats / filter bar / table / toolbar — matches the reference Blade pattern
 * across all 4 list views.
 */
...
interface VariantConfig {
  ...
  /** Optional extra filter (e.g. is_return=1 for returns) appended to every query. */
  extraFilter?: [string, any, unknown];
  /** Excluded extra filter (e.g. is_return=0 for invoices). */
  excludeFilter?: [string, any, unknown];
  /** Date field name. */
  dateField: 'posting_date' | 'transaction_date';
  /** Grand-total field name. */
  totalField: string;
}
...
export default function SalesDocumentListPage({ variant }: { variant: keyof typeof VARIANTS }) {
...
function Body({ cfg, variant }: { cfg: VariantConfig; variant: string }) {
  ...
  // Stat counts driven by the same base filter set.
  ...
  const filters = useMemo(() => {
    ...
    if (search.trim()) f.push([cfg.customerField, 'like', `%${search.trim()}%`]);
  ...
  const toolbarColumns: ToolbarColumn[] = useMemo(() => [
    ...
    { id: cfg.customerField, header: 'العميل' },
    { id: 'grand_total', header: 'الإجمالي' },
    { id: 'status', header: 'الحالة' },
  ], [cfg.dateField, cfg.customerField]);
      ...
      <div className="bg-white dark:bg-slate-800/50 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 p-3">
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="بحث برقم أو عميل..." className="w-full ps-3 pe-9 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg text-sm" />
          </div>
                ...
                <tr key={r.name} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-center gap-1">
                      <button type="button" onClick={() => navigate(`${cfg.basePath}/${encodeURIComponent(r.name)}`)} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-50 transition" aria-label="view">
                        <Eye size={16} />
                      </button>
                      <button type="button" onClick={() => navigate(`${cfg.basePath}/${encodeURIComponent(r.name)}/edit`)} className="p-1.5 rounded-lg text-slate-400 hover:text-[color:var(--color-brand-600)] hover:bg-[color:var(--color-brand-50,#ecfdf5)] transition" aria-label="edit">
                        <Pencil size={16} />
                      </button>
                      <button type="button" onClick={() => onDelete(r.name)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition" aria-label="delete">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
...
function StatusBadge({ docstatus, status }: { docstatus: 0 | 1 | 2; status?: string }) {
  // Prefer the doc-defined status (Sales Invoice has Paid/Overdue/etc.) when present.
  if (status && status !== 'Draft' && status !== 'Submitted' && status !== 'Cancelled') {
    const isGood = ['Paid', 'Completed', 'Delivered', 'To Bill'].includes(status);

/**
 * Payment Voucher (سند صرف) create / edit form.
 * Matches reference: delta-enterprise.net/financial/payment-vouchers/create
 * ERPNext doctype: Payment Entry (payment_type = "Pay")
 */
...
// ─── Page wrapper ─────────────────────────────────────────────────────────────
export default function PaymentVoucherFormPage({ mode }: { mode: 'create' | 'edit' }) {
  ...
  return (
    <RequirePerm doctype="Payment Entry" action="read">
      <PageShell
        title={isEdit ? `تعديل سند صرف: ${id ?? ''}` : 'سند صرف جديد'}
        subtitle="تسجيل سند صرف لدفع مبلغ"
        actions={
          <button
            type="button"
      ...
      >
        <Body mode={mode} name={id} onDone={() => navigate('/financial/payment-vouchers')} />
      </PageShell>
    </RequirePerm>
  );
}
...
function Body({
  ...
  // Pay: paid_to = payable account for Supplier, receivable for Customer refund.
  useEffect(() => {
    if (isEdit) return;
    if (doc.paid_to) return;
    ...
    >
      ...
      <div className="bg-white dark:bg-slate-800/50 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 overflow-hidden">
        <div className="px-6 py-4 flex items-center justify-between bg-gradient-to-l from-indigo-700 to-indigo-900">
          <h3 className="text-base font-bold text-white">بيانات سند الصرف</h3>
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            {/* Row 2 */}
            <Field label="المصروف إليه (إذا لم يكن مورد)">
              <input
                type="text"
